...
import org.xwiki.rendering.util.BlockSizeEstimator;
...
public class MacroTransformation extends AbstractTransformation implements Initializable
{
        ...
        for (int recursions = 0; recursions < this.maxRecursions;) {
            ...
            try {
                ...
                if (macroBlock.getParent() == null) {
                    ...
                    // Look for a limit that has been reached in the whole cause chain as macros usually wrap what
                    // they catch, so that the reached limit is reported instead of a generic macro failure.
                    MacroRenderingLimitExceededException limitException =
                        ExceptionUtils.throwableOfType(e, MacroRenderingLimitExceededException.class);

                    if (limitException == null) {
                        generateError(macroBlock, TM_FAILEDMACRO,
                            "Failed to execute the [{}] macro.", null, macroBlock.getId(), e);
                    } else {
                        // Note that this doesn't mean that the budget is exhausted: a single piece of content can be
                        // too large for what is left of it while smaller macros can still be executed. The loop is
                        // stopped by chargeExecution() when the budget is actually exhausted.
                        generateLimitError(macroBlock, limitException.getLimitType());
                    }
                }
    ...
    private RenderingLimitType chargeExecution()
    {
        ...
        // The size is charged after the execution of a macro, so it can also have been exhausted by a nested
        // transformation that shares the budget. Check it here, too, so that no further macro is executed whose result
        // would just be discarded, which matters as that result is kept in memory until it is discarded.
        if (this.renderingLimits.isExceeded(RenderingLimitType.DOCUMENT_SIZE)) {
            return RenderingLimitType.DOCUMENT_SIZE;
        }
